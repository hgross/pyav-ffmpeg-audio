...
- ``ffmpeg-windows-x86_64.tar.gz`` / ``ffmpeg-macos-arm64.tar.gz`` /
  ``ffmpeg-macos-x86_64.tar.gz`` / ``ffmpeg-manylinux-x86_64.tar.gz`` /
  ``ffmpeg-manylinux-aarch64.tar.gz`` — the FFmpeg dev trees (``bin``,
  ``include``, ``lib`` with import libraries and pkg-config files),
- ``av-16.1.0+*.whl`` — PyAV wheels (CPython 3.12/3.13; Windows x64, macOS
  arm64 + x86_64, Linux manylinux x86_64/aarch64) built against those
  libraries,
...
``windows-latest`` (MSYS2/MINGW64), ``macos-14`` (arm64),
``macos-15-intel`` (x86_64), and ``ubuntu-24.04``/``ubuntu-24.04-arm`` (where
FFmpeg is compiled inside the manylinux container via cibuildwheel so the
wheels stay glibc-portable). Deliberately not built: musllinux (PySide6/Qt
ship no musl wheels, so the consuming app cannot exist there). The wheel jobs
